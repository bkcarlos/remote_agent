...
The Gateway never opens workspace files directly. Every file operation starts an independent, short-lived `file-worker` process with a sanitized environment and a signed, scoped, 30-second capability. On Linux the child is created in new user, mount, network, PID, IPC, and UTS namespaces with a parent-death signal. After capability verification it applies `no_new_privs`, CPU/open-file/process/file-size/core rlimits, a seccomp-BPF filter, and a Landlock ruleset restricted to the workspace; read jobs receive no workspace write rights and no worker receives host network access. Worker stdout and stderr use bounded writers so output is rejected before it can grow Gateway memory without limit. Sandbox or resource-limit setup failure is fail-closed. The seccomp filter denies networking, mount, namespace mutation, ptrace, BPF, performance-event, kernel-module, keyring, reboot and chroot syscalls.
