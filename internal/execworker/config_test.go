...
func TestAdministratorConfigStrictAndCannotSetWorkspace(t *testing.T) {
	profile := `{"name":"go-test","executable":"/usr/bin/env","fixed_argv":["go","test"],"allowed_argv_prefixes":[["./..."]],"workspace_mode":"read_only","env_allowlist":["GOFLAGS"],"limits":{"timeout_ms":30000,"cpu_seconds":30,"memory_bytes":268435456,"pids":64,"output_bytes":1048576,"scan_regions":16,"scan_bytes":1048576,"scan_results":32}}`
	...
	if _, err := ParseAdministratorConfig([]byte(strings.Replace(valid, `"executable":"/usr/bin/env"`, `"executable":"env"`, 1))); err == nil {
